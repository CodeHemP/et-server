...
if not os.path.exists(LOCAL_NLLB_DIR):
    ...
    try:
        # We use TransformersConverter and explicitly allow safetensors 
        # to bypass the torch version check if possible.
        import ctranslate2.converters
        converter = ctranslate2.converters.TransformersConverter(
            OFFICIAL_NLLB_MODEL, 
            load_as_float16=True # Helps with memory during conversion
        )
        # Convert using INT8 for the 4090
        converter.convert(LOCAL_NLLB_DIR, quantization="int8", force=True)
        print("   ✅ Conversion Complete!")
    except Exception as e:
        print(f"   ❌ Conversion Failed: {e}")
        print("\n   ACTION REQUIRED:")
        print("   Please run: pip install torch==2.6.0 --index-url https://download.pytorch.org/whl/cu124")
        exit(1)
else:
...
async def transcribe_handler(websocket):
    ...
    try:
        async for message in websocket:
            ...
            if len(audio_buffer) >= int(SAMPLE_RATE * 2.5):
                # Simple VAD check
                tensor = torch.tensor(audio_buffer[-16000:])
                if vad_model(tensor, 16000).item() < 0.4:
